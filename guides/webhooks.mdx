...
`POST /v1/nesting/result/`. It carries the completed layout, per-sheet placements, and
...
- **Confirm out of band.** On receiving a webhook, you can independently re-fetch the result
  with its `RequestId` via `POST /v1/nesting/result/` using your API key. This confirms the
...
- Fall back to polling `POST /v1/nesting/result/` with the `RequestId`.
