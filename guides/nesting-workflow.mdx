...
<Steps>
  <Step title="Submit">
    ...
    The API validates the request and returns an `ApiNestingResult` — just `{ "RequestId": … }`,
    the id you track the job by. This acknowledgement is **not** the result.
  </Step>
  <Step title="Nest">
  ...
  <Step title="Deliver">
    On completion a **completion notification** is delivered to your webhook (and/or streamed
    over SignalR), carrying a summary and a `ResultIdentity`. You can also poll for it with the
    `RequestId`. See [Webhooks](/guides/webhooks) for the exact payload of each endpoint.
  </Step>
</Steps>
...
finishes, NestAPI `POST`s a completion notification to that URL — a summary plus the
`ResultIdentity` you use to fetch the full result or an export. This is the recommended
default for server-to-server integrations. See [Webhooks](/guides/webhooks) for the exact
payload shape (it differs by endpoint).
...
| `POST /v1/nesting/result/` | The full report-source result (`ReportSourceResponseV1`) — pass `RequestId` and `ResultIdentity` |
