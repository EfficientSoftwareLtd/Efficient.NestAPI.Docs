...
| Endpoint | For | Output |
|----------|-----|--------|
| `GET /v1/nesting/export` | A standard production nest | Nest export (geometry + reports) |
| `GET /v1/nesting/exportquotenest` | A [quote nest](/guides/quote-nesting) | Quote-oriented export |
| `GET /v1/nesting/render/` | Any completed nest | A rendered image of the layout |
...
- Producing cut files for a **won job** → `GET /v1/nesting/export`.
- Backing up a **quote** with a layout → `GET /v1/nesting/exportquotenest`.
- Showing the nest in a **UI** → `GET /v1/nesting/render/`.
