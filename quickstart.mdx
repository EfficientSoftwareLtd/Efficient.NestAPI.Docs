...
```bash
curl https://dev.api.nestapi.com/v1/nesting \
  -X POST \
  -H "Authorization: Bearer YOUR_API_KEY" \
...
When the nest finishes, NestAPI `POST`s a completion notification to your webhook — a summary
(efficiency, sheet and part counts) plus a `Status.ResultIdentity`. It is **not** the full
layout; use that `ResultIdentity` to fetch the complete result. See
[Webhooks](/guides/webhooks) for the exact payload.

Fetch the full report-source result with the `RequestId` and the `ResultIdentity` from the
webhook:
...
```bash
curl "https://dev.api.nestapi.com/v1/nesting/export?RequestId=THE_REQUEST_ID" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
